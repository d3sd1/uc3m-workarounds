			...
			if( Object.keys(errors).length > 0 ) {
				...
				if(form.attr('id') == 'contacts_form') {
            		...
            		jQuery.post('subscribe.php', post_data, function(response){
        		        output = '<div class="success">Te has suscrito correctamente.</div>';
        		        //reset values in all input fields
        		        $("#contacts_form .form_item").val('');
...
function showErrors(form, errors) {
	...
	for(var i in errors) {
		...
		if( errors[i] == 'empty' )
			error_message += '<div class="error">Introduce un email.</div>';
		else
			error_message += '<div class="error">Introduce un email válido.</div>';
	}
