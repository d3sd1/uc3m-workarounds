...
if(!isset($_GET['type']) || !isset($_GET['langkey']) || !array_key_exists(@$_GET['langkey'],array_flip(explode(',',$config['avaliable.langs']))))
{
            ...
            <div class="page-content page-content-popup">
                <div class="page-content-fixed-header">
                    ...
                    <ul class="page-breadcrumb">
						...
						<li><?php echo $lang['path.langs'] ?></li>
                    </ul>
                    <!-- END BREADCRUMBS -->
                ...
                <div class="page-fixed-main-content">
					<h1><?php echo $lang['langedit.addrow.h1'].': <b>'.$_GET['type'].' '.strtoupper($_GET['langkey']).'</b>' ?></h1>
					<?php
					if(@$_POST['addNewLangKey'] == "true")
					{
						if(isset($_POST['addRowKey']) && isset($_POST['addRowVal']) && $_POST['addRowKey'] != null && $_POST['addRowVal'] != null)
						{
							$loadLang = json_decode(file_get_contents(BASEDIR.'/kernel/langs/'.(($_GET['type'] == 'admin') ? 'admin/':null).$_GET['langkey'].'.json'),true);
							$_POST['addRowKey'] = str_replace(array('_','<x>',' ','/'),array('.',null,null,null),$_POST['addRowKey']);
							if(array_key_exists($_POST['addRowKey'],$loadLang))
							{
								echo '<div class="alert alert-danger fade in alert-dismissable" style="margin-top:18px;">
									<a href="#" class="close" data-dismiss="alert" aria-label="close" title="close">×</a>
									<strong>'.$lang['langaddrow.error.title'].'</strong> '.$lang['langaddrow.error.duplidesc'].'
								</div>';
							}
							else
							{
								$loadLang[$_POST['addRowKey']] = $_POST['addRowVal'];
								$parseDots = json_encode($loadLang);
								foreach(explode(',',$parseDots) as $getRow)
								{
									$oldVal = explode(':',$getRow)[0];
									$newVal = str_replace(array('_','<x>'),array('.',null),$oldVal);
									$parseDots = str_replace($oldVal,$newVal,$parseDots);
								}
								file_put_contents(BASEDIR.'/kernel/langs/'.(($_GET['type'] == 'admin') ? 'admin/':null).$_GET['langkey'].'.json',$parseDots);
								echo '<div class="alert alert-success fade in alert-dismissable" style="margin-top:18px;">
									<a href="#" class="close" data-dismiss="alert" aria-label="close" title="close">×</a>
									<strong>'.$lang['langedit.success.title'].'</strong> '.$lang['langedit.success.desc'].'
								</div>';
							}
						}
						else
						{
							echo '<div class="alert alert-danger fade in alert-dismissable" style="margin-top:18px;">
								<a href="#" class="close" data-dismiss="alert" aria-label="close" title="close">×</a>
								<strong>'.$lang['langaddrow.error.title'].'</strong> '.$lang['langaddrow.error.desc'].'
							</div>';
						}
					}
					?>
					<form method="post">
					<input type="hidden" name="addNewLangKey" value="true">
					<div class="col-xs-6">
					  <label for="ex3"><?php echo $lang['langedit.addrow.key'] ?></label>
					  <input class="form-control" name="addRowKey" type="text">
					</div>
					<div class="col-xs-6" style="margin-bottom: 15px">
					  <label for="ex3"><?php echo $lang['langedit.addrow.val'] ?></label>
					  <input class="form-control" name="addRowVal" type="text">
					</div>
					<button style="float: right" type="submit" class="btn btn-primary"><?php echo $lang['langedit.addrow.send'] ?></button><br><br>
					</form>
					<h1><?php echo $lang['langedit.h1'].': <b>'.$_GET['type'].' '.strtoupper($_GET['langkey']).'</b>' ?></h1>
					<?php
					if(isset($_GET['deleteReg']))
					{
						$_GET['deleteReg'] = str_replace(array('_','<x>'),array('.',null),$_GET['deleteReg']);
						$loadLang = json_decode(file_get_contents(BASEDIR.'/kernel/langs/'.(($_GET['type'] == 'admin') ? 'admin/':null).$_GET['langkey'].'.json'),true);
						if(array_key_exists($_GET['deleteReg'],$loadLang))
						{
							unset($loadLang[$_GET['deleteReg']]);
							file_put_contents(BASEDIR.'/kernel/langs/'.(($_GET['type'] == 'admin') ? 'admin/':null).$_GET['langkey'].'.json',json_encode($loadLang));
							echo '<div class="alert alert-success fade in alert-dismissable" style="margin-top:18px;">
									<a href="#" class="close" data-dismiss="alert" aria-label="close" title="close">×</a>
									<strong>'.$lang['langdel.success.title'].'</strong> '.$lang['langdel.success.desc'].'
								</div>';
						}
						else
						{
							echo '<div class="alert alert-danger fade in alert-dismissable" style="margin-top:18px;">
								<a href="#" class="close" data-dismiss="alert" aria-label="close" title="close">×</a>
								<strong>'.$lang['langdelrow.error.title'].'</strong> '.$lang['langdelrow.error.desc'].'
							</div>';
						}
					}
					if(isset($_POST['updateLangFile']) == "true")
					{
						$oldLoadLang = json_decode(file_get_contents(BASEDIR.'/kernel/langs/'.(($_GET['type'] == 'admin') ? 'admin/':null).$_GET['langkey'].'.json'),true);
						unset($oldLoadLang['']);
						unset($_POST['updateLangFile']);
						$newLoadLang = $_POST;
						if(count($newLoadLang) == count($oldLoadLang))
						{
							$parseDots = json_encode($newLoadLang);
							foreach(explode(',',$parseDots) as $getRow)
							{
								$oldVal = explode(':',$getRow)[0];
								$newVal = str_replace(array('_','<x>'),array('.',null),$oldVal);
						...
						else
						{
							...
							unset($_POST['updateLangFile']);
							$newLoadLang = $_POST;
							$finalLoadLang = array_merge($oldLoadLang,$newLoadLang);
							$parseDots = json_encode($finalLoadLang);
							foreach(explode(',',$parseDots) as $getRow)
							{
								$oldVal = explode(':',$getRow)[0];
								$newVal = str_replace(array('_','<x>'),array('.',null),$oldVal);
							...
							file_put_contents(BASEDIR.'/kernel/langs/'.(($_GET['type'] == 'admin') ? 'admin/':null).$_GET['langkey'].'.json',$parseDots,JSON_NUMERIC_CHECK);
							echo '<div class="alert alert-success fade in alert-dismissable" style="margin-top:18px;">
								<a href="#" class="close" data-dismiss="alert" aria-label="close" title="close">×</a>
								<strong>'.$lang['langedit.success.title'].'</strong> '.$lang['langedit.success.desc'].'
							</div>';
					...
					<form method="post">
					<br>
					<input type="hidden" name="updateLangFile" value="true">
                    <?php 
						...
						if($key != null)
						{
							echo '<div class="input-group">
							  ...
							  <div class="input-group-btn">
								<a href="'.URL.'/langsMgr/'.(($_GET['type'] == 'admin') ? 'admin':'web').'/'.$_GET['langkey'].'/'.$key.'"><button type="button" class="btn btn-danger" style="margin-top: 24px">'.$lang['langedit.delete'].'</button></a>
							  </div>
							</div><br>';
						}
					}
					?>
					<button style="float: right" type="submit" class="btn btn-primary"><?php echo $lang['langedit.send'] ?></button>
					</form>
                </div>
                <!-- BEGIN FOOTER -->
